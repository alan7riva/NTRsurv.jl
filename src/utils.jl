function surv_mat(l::Int64,t::Vector{Float64},α::Float64,baseline::BaselineNTR)
    S_mat = zeros(Float64, length(t), l)
    for i in 1:l
        S_mat[:,i] = prior_sim(t,α,baseline)
    end
    return S_mat
end
...
function cred_band( p::Float64, l::Int64, t::Vector{Float64}, z_new::Vector{Float64}, model::ModelRegreNTR,μ::Bool=true)
    if  !( 0 < p < 1)
        @error "ERROR: p is not between zero and one."
    end
    m = round( Int, l*p)
