@inline function random_walk_mh_step( lf::Function, s::Float64, lfs::Float64, prop_σ::Float64)
...
@inline function random_walk_mh_within_blocked_gibbs_step( lf::Vector{Function}, blocks::Vector{UnitRange{Int64}}, l::Int64, d::Vector{Int64}, s::Vector{Float64}, lfs::Vector{Float64}, prop_σ::Vector{Float64})
    for i in 1:l
        s_block = s[ blocks[i] ]
        prop_σ_block = prop_σ[ blocks[i] ]
        for j in 1:d[i]
            s[ blocks[i][j] ], lfs[i] = random_walk_mh_step( x -> lf[i]( [s_block[1:(j-1)];x;s_block[(j+1):d[i]]] ), s_block[j], lfs[i], prop_σ_block[j])
        end
    end
    return s, lfs
end
...
@inline function random_walk_mh( b::Int64, lf::Function, s::Float64, lfs::Float64, prop_σ::Float64)
    ...
    lfs_run = lfs
    for j in 2:b
        s_run, lfs_run = random_walk_mh_step( lf, s_run, lfs_run, prop_σ)
...
@inline function random_walk_mh_within_gibbs( b::Int64, lf::Function, s::Vector{Float64}, lfs::Float64, prop_σ::Vector{Float64})
    ...
    lfs_run = lfs
    for j in 2:b
        s_run, lfs_run = random_walk_mh_within_gibbs_step( lf, d, s_run, lfs_run, prop_σ)
        chain_s[j] = copy(s_run)
    end
    return chain_s, lfs_run
end

@inline function random_walk_mh_within_blocked_gibbs( b::Int64, lf::Vector{Function}, blocks::Vector{UnitRange{Int64}}, s::Vector{Float64}, lfs::Vector{Float64}, prop_σ::Vector{Float64})
    l = length(blocks)
    d = length.(blocks)
    ...
    lfs_run = lfs
    for j in 2:b
        s_run, lfs_run = random_walk_mh_within_blocked_gibbs_step( lf, blocks, l, d, s_run, lfs_run, prop_σ)
        chain_s[j] = copy(s_run)
    end
    return chain_s, lfs_run
end
...
function acceptance_rate(v::Vector{Vector{Float64}})
    l = length(v)
    d = length(v[1])
    rej = zeros(d)
    for i in 1:d
...
@inline function robbins_monro_mh_tune_with_progress(n::Int64,b::Int64,lf::Function,s₀::Float64,σ₀::Float64,
    ...
    for i in 2:(n+1)
        c_s_tmp, lfs_run = random_walk_mh( b, lf, s_run, lfs_run, c_σ[i-1])
        s_run = c_s_tmp[end]
        c_θ[i] = c_θ[i-1] + (1/i^γ)*( acceptance_rate(c_s_tmp) - p_acc )
        c_σ[i] = exp(c_θ[i])
        next!(prog)
    end
...
@inline function robbins_monro_mh_tune_without_progress(n::Int64,b::Int64,lf::Function,s₀::Float64,σ₀::Float64,
        p_acc::Float64,γ::Float64)
    s_run = s₀
    lfs_run = lf(s₀)
    c_σ = zeros(n+1) 
...
function robbins_monro_mh_tune(n::Int64,b::Int64,lf::Function,s₀::Float64,σ₀::Float64,
    ...
    if show_progress
        ...
        return robbins_monro_mh_tune_without_progress(n,b,lf,s₀,σ₀,p_acc,γ)
    end
end

@inline function robbins_monro_mh_within_gibbs_tune_with_progress(n::Int64,b::Int64,lf::Function,s₀::Vector{Float64},σ₀::Vector{Float64},
    ...
    prog = Progress( n, dt=0.5, barglyphs=BarGlyphs("[=> ]"), barlen=50)
    for i in 2:(n+1)
        c_s_tmp, lfs_run = random_walk_mh_within_gibbs( b, lf, s_run, lfs_run, c_σ[i-1])
        s_run = c_s_tmp[end]
        c_θ[i] = c_θ[i-1] .+ (1/i^γ).*( acceptance_rate(c_s_tmp) .- p_acc )
        c_σ[i] = exp.(c_θ[i])
        next!(prog)
...
@inline function robbins_monro_mh_within_blocked_gibbs_tune_with_progress( n::Int64, b::Int64, lf::Vector{Function}, blocks::Vector{UnitRange{Int64}}, s₀::Vector{Float64}, σ₀::Vector{Float64},
        p_acc::Vector{Float64},γ::Float64)
    d = length(s₀)
    s_run = s₀
    ...
    prog = Progress( n, dt=0.5, barglyphs=BarGlyphs("[=> ]"), barlen=50)
    for i in 2:(n+1)
        c_s_tmp, lfs_run = random_walk_mh_within_blocked_gibbs( b, lf, blocks, s_run, lfs_run, c_σ[i-1])
        s_run = c_s_tmp[end]
        c_θ[i] = c_θ[i-1] .+ (1/i^γ).*( acceptance_rate(c_s_tmp) .- p_acc )
        c_σ[i] = exp.(c_θ[i])
        next!(prog)
...
@inline function robbins_monro_mh_within_blocked_gibbs_tune_without_progress( n::Int64, b::Int64, lf::Vector{Function}, blocks::Vector{UnitRange{Int64}}, s₀::Vector{Float64}, σ₀::Vector{Float64},
        p_acc::Vector{Float64},γ::Float64)
    d = length(s₀)
    s_run = s₀
    s₀_blocks = [ s₀[i] for i in blocks ]
...
function robbins_monro_mh_within_blocked_gibbs_tune( n::Int64, b::Int64, lf::Vector{Function}, blocks::Vector{UnitRange{Int64}}, s₀::Vector{Float64},σ₀::Vector{Float64},
    ...
    if show_progress
        ...
        return robbins_monro_mh_within_blocked_gibbs_tune_without_progress( n, b, lf, blocks, s₀, σ₀, p_acc, γ)
    end
end
